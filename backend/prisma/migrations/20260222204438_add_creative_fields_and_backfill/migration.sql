...
-- AlterTable
ALTER TABLE `creatives` ADD COLUMN `description` TEXT NULL,
    ADD COLUMN `icon_url` TEXT NULL,
    ADD COLUMN `type` VARCHAR(191) NOT NULL DEFAULT 'IN_PAGE_PUSH';

-- CreateTable
CREATE TABLE `backfill_impressions` (
    ...
    `network_revenue` DOUBLE NOT NULL DEFAULT 0,
    `publisher_revenue` DOUBLE NOT NULL DEFAULT 0,
    `system_profit` DOUBLE NOT NULL DEFAULT 0,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    INDEX `backfill_impressions_zone_id_idx`(`zone_id`),
    INDEX `backfill_impressions_created_at_idx`(`created_at`),
    PRIMARY KEY (`id`)
