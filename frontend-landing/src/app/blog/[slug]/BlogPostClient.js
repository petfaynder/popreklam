...
function injectIds(html) {
    if (!html) return '';
    let i = 0;
    // Replace h2/h3 opening tags with id attributes for TOC anchors
...
function ShareSidebar({ title }) {
    ...
    return (
        <div className="hidden xl:flex flex-col items-center gap-2 sticky top-28 self-start pt-2">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground [writing-mode:vertical-lr] rotate-180 mb-1">Share</span>
            {[
...
function RelatedCard({ post }) {
    ...
    return (
        <Link href={`/blog/${post.slug}`} className="group flex gap-4 border-b-2 border-foreground/10 last:border-0 py-4 hover:bg-foreground/3 px-2 -mx-2 transition-colors">
            <div className="relative w-20 h-16 shrink-0 overflow-hidden border border-foreground/20" style={{ background: grad }}>
                {post.coverImage
            ...
            <div className="flex-1 min-w-0">
                <span className="text-[9px] font-black uppercase tracking-widest block mb-1" style={{ color: c.pill }}>{post.primaryCategory}</span>
                <p className="text-sm font-black uppercase tracking-tight leading-snug line-clamp-2 group-hover:text-primary transition-colors">{post.title}</p>
...
function ArticleSchema({ article }) {
    const schema = {
        ...
        image: article.coverImage || 'https://mrpop.io/og-image.png',
        ...
        publisher: {
            '@type': 'Organization',
            name: 'MrPop.io',
            logo: { '@type': 'ImageObject', url: 'https://mrpop.io/logo.png' },
        },
        mainEntityOfPage: {
            '@type': 'WebPage',
            '@id': `https://mrpop.io/blog/${article.slug}`,
        },
    };
    return (
        <script
                ...
                <div className="bg-background">
                    ...
                    <div className="border-b border-foreground/10">
                        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                            ...
                            <ChevronRight className="w-3 h-3 opacity-30" />
                            <span className="font-black uppercase tracking-wider" style={{ color: c.pill }}>{article.primaryCategory}</span>
                        </div>
                    </div>
                    ...
                    <div className="relative w-full overflow-hidden border-b-4 border-foreground" style={{ minHeight: '400px' }}>
                        {article.coverImage
                            ? <img src={article.coverImage} alt={`${article.title} - MrPop.io Blog`} className="absolute inset-0 w-full h-full object-cover" />
                            : (
                        ...
                        <div className="relative max-w-5xl mx-auto px-8 pt-20 pb-10 flex flex-col justify-end min-h-[400px]">
                            <div className="flex flex-wrap gap-2 mb-5">
                                ...
                                {article.tags.slice(0, 2).map(t => (
                                    <span key={t} className="px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider border border-white/30 text-white/80">{t}</span>
                                ))}
                            </div>
                    ...
                    <div className="border-b border-foreground/10 bg-foreground/[0.025]">
                        <div className="max-w-5xl mx-auto px-8 py-6">
                            <p className="text-lg font-medium text-muted-foreground leading-relaxed border-l-4 pl-5" style={{ borderColor: c.pill }}>
                                {article.excerpt}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto px-6 py-10">
                        <div className="flex gap-8 xl:gap-12 items-start">
                            ...
                            <article className="flex-1 min-w-0 max-w-3xl">
                                <div className="blog-prose" dangerouslySetInnerHTML={{ __html: injectIds(article.content) }} />
                                {article.tags.length > 0 && (
                                    <div className="mt-12 pt-8 border-t-2 border-foreground/20 flex flex-wrap gap-2 items-center">
                                        <Tag className="w-4 h-4 text-muted-foreground shrink-0" />
                                        {article.tags.map(t => (
                                            <span key={t} className="px-3 py-1.5 border-2 border-foreground text-xs font-black uppercase hover:bg-foreground hover:text-background transition-all cursor-pointer">{t}</span>
                                        ))}
                                    </div>
                                )}
                            ...
                            <aside className="hidden lg:block w-72 shrink-0">
                                <div className="sticky top-24 space-y-5">
                                    ...
                                    <div className="border-2 border-foreground bg-foreground text-background p-5 relative overflow-hidden">
                                        ...
                                        <h3 className="text-sm font-black uppercase tracking-tight leading-tight mb-2">Maximize Your Ad Revenue</h3>
                                        <p className="text-xs text-background/50 mb-4 leading-relaxed font-medium">10,000+ publishers trust MrPop.io for premium monetization.</p>
                                        <Link href="/register" className="block w-full py-2.5 bg-primary text-white text-xs font-black uppercase tracking-widest text-center hover:bg-accent transition-colors">Get Started Free →</Link>
                                        <Link href="/for-advertisers" className="block w-full py-2 mt-1 text-[10px] font-bold uppercase tracking-widest text-center text-background/40 hover:text-background transition-colors">I'm an Advertiser</Link>
                                    </div>
                                    <div className="border-2 border-foreground overflow-hidden">
                                        <div className="border-b-2 border-foreground bg-foreground text-background px-4 py-3 flex items-center gap-2">
                            ...
                            <div className="max-w-7xl mx-auto px-6 py-12">
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h2 className="text-xl font-black uppercase tracking-tighter">More in {article.primaryCategory}</h2>
                                    </div>
                                    <Link href={`/blog?category=${article.primaryCategorySlug}`} className="text-xs font-black uppercase tracking-widest hover:text-primary transition-colors flex items-center gap-1">
                                        ...
                                        return (
                                            <Link key={post.id} href={`/blog/${post.slug}`}
                                                ...
                                                <div className="relative h-36 overflow-hidden" style={{ background: pg }}>
                                                    {post.coverImage
                                                        ? <img src={post.coverImage} alt={`${post.title} - MrPop.io Blog`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-80" />
                                                        : <>
                                                ...
                                                <div className="p-4">
                                                    <h3 className="text-sm font-black uppercase tracking-tight leading-snug line-clamp-2 group-hover:text-primary transition-colors mb-2">{post.title}</h3>
                                                    <p className="text-xs text-muted-foreground flex items-center gap-2 font-bold">
                                                        <span>{post.dateFormatted}</span>
                                                        <span className="text-primary flex items-center gap-1"><Clock className="w-3 h-3" />{post.readTime}</span>
                                                    </p>
                                                </div>
                                            </Link>
                                        );
                    ...
                    <section className="border-t-4 border-foreground bg-foreground text-background">
                        <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-8 items-center justify-between">
                            ...
                            <div className="flex gap-3 w-full md:max-w-md">
                                ...
                                <button className="px-6 py-3 bg-primary text-white font-black uppercase text-sm whitespace-nowrap">Subscribe →</button>
                            </div>
                        </div>
                    </section>
