                    ...
                    <div className="py-20 px-4 md:px-8 max-w-7xl mx-auto">
                        ...
                        <div className="mb-16 text-center md:text-left">
                            <h1 className={`text-5xl md:text-7xl font-black leading-none ${isBrutalist ? 'uppercase tracking-tighter' : isEditorial ? 'tracking-tight' : 'tracking-tight'}`}>
                                {isBrutalist ? <><span className="text-outline text-transparent">{t('title')}</span></> : t('title')}
                            </h1>
                            ...
                            <div className={`${cardBg} p-8 md:p-12 relative`}>
                                ...
                                <form className="space-y-6">
                                    <div className="grid md:grid-cols-2 gap-6">
                                        <div className="space-y-2"><label className={labelCls}>{t('nameLabel')}</label><input type="text" className={inputCls} placeholder={t('namePlaceholder')} /></div>
                                        <div className="space-y-2"><label className={labelCls}>{t('emailLabel')}</label><input type="email" className={inputCls} placeholder={t('emailPlaceholder')} /></div>
                                    </div>
                                    <div className="space-y-2"><label className={labelCls}>{t('subjectLabel')}</label><input type="text" className={inputCls} placeholder={t('subjectPlaceholder')} /></div>
                                    <div className="space-y-2"><label className={labelCls}>{t('messageLabel')}</label><textarea rows="4" className={inputCls} placeholder={t('messagePlaceholder')}></textarea></div>
                                    <button className={btnCls}>{t('sendBtn')}</button>
                                </form>
                            </div>
