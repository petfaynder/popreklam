...
export default function PublisherSitesPage() {
    ...
    const columns = [
        {
            header: t('siteName'), accessor: 'name', sortable: true,
            render: (row) => (
                <div>
        ...
        { header: t('category'), accessor: 'category', sortable: true },
        { header: tCommon('status'), accessor: 'status', sortable: true, render: (row) => statusBadge(row.status) },
        { header: t('impressionsToday'), accessor: 'impressions', sortable: true, render: (row) => (row.impressions || 0).toLocaleString() },
        { header: t('earningsToday'), accessor: 'revenue', sortable: true, render: (row) => `$${Number(row.revenue || 0).toFixed(2)}` },
        { header: tCommon('date'), accessor: 'createdAt', sortable: true, render: (row) => new Date(row.createdAt).toLocaleDateString() },
        {
        ...
        return (
            <div className="flex items-center justify-center h-96">
                <div className="text-center">
                    ...
                    <p className={d.loaderText}>{tCommon('loading')}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className={`${d.heading} mb-2`}>{t('title')}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <StatsCard icon={Globe} title={t('title')} value={sites.length} subtitle={`${activeSites} active`} color="lime" />
                <StatsCard icon={Eye} title={t('impressionsToday')} value={totalImpressions.toLocaleString()} subtitle="Last 30 days" color="sky" />
                <StatsCard icon={DollarSign} title={t('earningsToday')} value={`$${totalRevenue.toFixed(2)}`} subtitle="Last 30 days" color="purple" />
            ...
            {/* Add Site Modal */}
            <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title={t('addSite')} size="medium" variant={theme}>
                ...
                <form onSubmit={handleAddSite} className="space-y-6">
                    <div>
                        <label className={d.labelCls}>{t('siteUrl')} *</label>
                    ...
                    <div>
                        <label className={d.labelCls}>{t('siteName')} *</label>
                    ...
                    <div>
                        <label className={d.labelCls}>{t('category')} *</label>
                    ...
                    <div className="flex gap-4">
                        <button type="button" onClick={() => setIsAddModalOpen(false)} disabled={submitting} className={`flex-1 ${d.btnSecondary} disabled:opacity-50`}>{tCommon('cancel')}</button>
            ...
            {/* Edit Site Modal */}
            <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title={tCommon('edit')} size="medium" variant={theme}>
                <form onSubmit={handleUpdateSite} className="space-y-6">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button type="submit" className={`flex-1 ${d.btnPrimary}`}>{tCommon('save')}</button>
                    </div>
                </form>
            </Modal>
