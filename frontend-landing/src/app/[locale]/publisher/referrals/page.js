...
import {
    Gift, Copy, Check, Users, TrendingUp, DollarSign,
...
export default function PublisherReferralsPage() {
    ...
    const load = useCallback(async () => {
        try {
            ...
            const [i, s, r] = await Promise.all([
                publisherAPI.getReferralInfo(),
                publisherAPI.getReferralStats(),
                publisherAPI.getReferrals({ limit: 50 }),
            ]);
            setInfo(i);
            setStats(s);
    ...
    const statCards = [
        ...
        {
            label: 'Total Earned', value: stats ? `$${Number(stats.totalEarned).toFixed(2)}` : '—', icon: DollarSign,
            color: 'text-sky-500', bg: 'bg-sky-500/10 border border-sky-500/20'
        },
        {
            label: 'Pending Earnings', value: stats ? `$${Number(stats.pendingEarnings).toFixed(2)}` : '—', icon: Clock,
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className={`${d.subheading} mt-1`}>
                        Invite publishers and earn {info?.publisherCommission ?? 5}% commission on their earnings
                    </p>
                </div>
            ...
            <div className={`${d.card} relative overflow-hidden`}>
                ...
                <div className="relative flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p className={`text-xs ${d.subheading}`}>Share this link to invite new publishers</p>
                    </div>
                </div>
                ...
                {info?.referralCode && (
                    <p className={`text-xs mt-3 ${d.subheading}`}>
                        ...
                        {info.publisherCommission}% commission on referred publisher earnings
                    </p>
                )}
            </div>
            ...
            <div className={d.card}>
                <h2 className={`font-bold text-sm mb-4 ${d.isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>Your Referrals</h2>
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className={`font-semibold text-sm ${d.isDark ? 'text-white' : 'text-[#1A1A1A]'}`}>{t('noReferrals')}</p>
                        <p className={`text-xs mt-1 ${d.subheading}`}>Share your link to start earning!</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className={d.tableHead}>
                                    {['Email', 'Status', 'Commission', 'Earned', 'Joined'].map(h => (
            ...
            <div className={d.card}>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    {[
                        { step: '01', icon: Link2, title: 'Share Your Link', desc: 'Copy your unique referral link and share it with other website owners and publishers.' },
                        { step: '02', icon: Users, title: 'They Register', desc: "When someone signs up using your link, they're linked to your account automatically." },
                        { step: '03', icon: DollarSign, title: 'You Earn', desc: `Earn ${info?.publisherCommission ?? 5}% commission on their ad revenue, paid to your balance.` },
                    ].map(item => (
                        <div key={item.step} className={`flex gap-3 p-4 rounded-xl ${d.isDark ? 'bg-white/[0.03] border border-white/8' : 'bg-gray-50 border border-gray-100'}`}>
                            <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black bg-violet-500/15 text-violet-500`}>
                                {item.step}
                            </div>
                            <div>
