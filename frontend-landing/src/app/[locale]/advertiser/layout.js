...
export default function AdvertiserLayout({ children }) {
    const t = useTranslations('advertiser.nav');
    ...
    const fetchNotifications = async () => {
        try {
            const data = await advertiserAPI.getNotifications();
    ...
    const handleNotificationClick = async (notif) => {
        if (!notif.isRead) {
            try {
                await advertiserAPI.markNotificationAsRead(notif.id);
        ...
        <div className={`${d.shell} selection:bg-sky-400 selection:text-slate-900`}>
            ...
            <aside className={`${d.sidebar} ${mobileOpen ? '!flex' : ''}`}>
                ...
                <nav className={d.sidebarNav}>
                    ...
                    <div className={d.sidebarSection}>{t('account')}</div>
                ...
                <div className={d.sidebarProfile}>
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className={d.avatarSub}>{tCommon('active')}</div>
                        </div>
            ...
            <div className="flex-1 lg:ml-64 flex flex-col min-h-screen relative z-10">
                ...
                <header className={d.topbar}>
                    ...
                    <div className={`hidden md:block ${d.breadcrumb}`}>
                        {t('dashboard')} &gt; <span className={d.breadcrumbActive}>
                            {pathname === '/advertiser' ? t('dashboard') : pathname.split('/').pop()?.charAt(0).toUpperCase() + pathname.split('/').pop()?.slice(1)}
                        </span>
                    </div>

                    <div className="flex items-center gap-4 md:gap-6 ml-auto">
                        {/* Language Switcher */}
                        <LanguageSwitcher dark={d.isDark} />
                        {/* Tier Badge */}
                        {tierData && (
                            ...
                            {bellOpen && (
                                <div className="absolute top-12 right-0 w-80 bg-white dark:bg-[#0f0f24] border border-gray-200 dark:border-gray-800 rounded-xl shadow-2xl z-50 overflow-hidden">
                                    <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-black/20">
                                        <h3 className="font-semibold text-gray-900 dark:text-white">{tCommon('noData')}</h3>
                                    </div>
                                    <div className="max-h-[400px] overflow-y-auto">
                                        {notifications.length === 0 ? (
                                            <div className="p-8 text-center text-gray-500 dark:text-gray-400">
                                                <Bell className="w-8 h-8 mx-auto mb-3 opacity-20" />
                                                <p className="text-sm">{tTopbar('noNotifications')}</p>
                                            </div>
                                        ) : (
                                            <div className="divide-y divide-gray-100 dark:divide-gray-800">
                                                {notifications.map(notif => (
                                                    <div 
                                                        ...
                                                        className={`p-4 cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-white/5 ${!notif.isRead ? 'bg-sky-50 dark:bg-sky-500/10' : ''}`}
                                                    >
